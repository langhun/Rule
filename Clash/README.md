...
这一轮继续只推 `Sub-Store.js` 主线，不碰固定模板，重点是把 **GitHub 社区常见的“美加墨 / 波罗的海 / 南锥体”这类子区域写法继续补进来**，同时把优先链解析那段再补一轮逐行中文注释。

1. **新增 3 组更社区化的子区域 token**
   - 新增：
     - `baltics`
     - `nafta`
     - `southerncone`
   - 对应的直觉就是：
     - `波罗的海三国`
     - `美加墨`
     - `阿根廷 / 智利 / 巴拉圭 / 乌拉圭` 这组南锥体

2. **同步补 3 个可直接复用的 Prefer-Countries preset**
   - 新增：
     - `baltics-core`
     - `nafta-core`
     - `southerncone-core`
   - 这样 GitHub / Steam / Dev 这类独立组现在可以直接短写：
     - `githubPreferCountries=nafta-core`
     - `steamPreferCountries=southerncone-core`
     - `devPreferCountries=baltics-core`
...
4. **逐行中文注释继续补**
   - 这次继续补到了：
     - `inspectPreferredProxyReference(...)`
     - `resolvePreferredRegionCountryGroups(...)`
     - `resolvePreferredCountryGroupEntriesByMarker(...)`
   - 现在你后面自己排查：
     - `preset` 是怎么递归展开的
     - `region token` 是怎么落到当前国家组的
     - `点名节点` 为什么是精确命中 / 模糊命中 / 歧义
   - 会更容易顺着日志和源码一起看

5. **这一轮可以直接这样用**

```text
...?target=ClashMeta&full&responseHeaders=true&regionGroups=baltics,nafta,southerncone&groupOrderPreset=dashboard&githubPreferCountries=nafta-core&steamPreferCountries=southerncone-core&devPreferCountries=baltics-core
```

```text
...?target=ClashMeta&full&responseHeaders=true&regionGroups=easteurope,baltics,nafta&groupOrderPreset=geo-compact&githubPreferCountries=classic-6&steamPreferCountries=nafta-core&devPreferCountries=workspace-core,baltics-core
```
...
这一轮继续沿着 **“面板更顺手 + 小而精的地区 preset 更好复用 + 固定模板顺序也一起理顺”** 往下做，重点是 **新增 dashboard 布局 + 再补一组常用地理 preset + 收一下 Rule.ini 的固定面板顺序**：
...
2. **短小地理 preset 继续补强**
   - 新增：
     ...
     - `oceania-core`
   - 这样像下面这些组合就能更短地写：
     - `githubPreferCountries=cjk-core`
     - `steamPreferCountries=asean-core`
     - `devPreferCountries=oceania-core`

3. **区域别名继续补一轮**
   - 这一轮继续补了：
     - `oceania` 兼容 `anz / anzac / 澳新`
   - 配合上一轮的：
     - `cjk`
     - `asean`
     - `five-eyes`
   - 现在从 GitHub 抄常见参数名时更不容易撞写法差异

4. **Rule.ini 固定模板顺序继续优化**
   - 这次把面板里的：
     - `🎯 全球直连`
   - 前移到：
     - `节点选择 / 手动切换 / 自动选择`
...
5. **逐行中文注释继续补**
   - 这次继续补到了：
     - `buildProxyGroupOrderBuckets(...)`
   - 这样后面你自己继续调：
     - `groupOrderPreset`
     - `groupOrder`
     - `regions / countries / helpers / extras`
   - 会更容易看懂每个 bucket 到底怎么展开

6. **这一轮可以直接这样用**

```text
...?target=ClashMeta&full&responseHeaders=true&regionGroups=cjk,asean,oceania&groupOrderPreset=dashboard&githubPreferCountries=cjk-core&steamPreferCountries=asean-core&devPreferCountries=oceania-core
```

```text
...?target=ClashMeta&full&responseHeaders=true&regionGroups=levant,westasia,mediterranean&groupOrderPreset=daily&githubPreferCountries=levant-core&steamPreferCountries=westasia-core&devPreferCountries=workspace-core
```
...
这一轮继续把“更多区域/国家玩法 + 默认面板布局”往下补厚，重点做的是 **语义修正 + 新区域 token + 更顺手的 preset 组合**：

1. **国家覆盖继续补洞**
   - 这次继续补了：
     - `老挝`
     - `巴拿马`
   - 主要是为了让中美洲 / 中转类命名不再轻易掉进：
     - `🐟 兜底节点`
   - 同时它也已经接进：
     - `americas`
     - `northamerica`
...
2. **区域 token 继续扩容**
   - 这次继续补了：
     ...
     - `dach`
     - `anglosphere`
   - 现在这套区域玩法已经不只是大洲拆分，还能更贴近日常机场选线和手动面板切换

3. **Prefer-Countries preset 继续补厚**
   - 这次继续补了：
     - `apac-core`
     - `westasia-core`
     - `greaterchina-core`
...
4. **国家优先布局语义修正**
   - 这一轮顺手修正了一个容易误导的点：
     - `groupOrderPreset=country`
     - `groupOrderPreset=countries`
   - 现在会真正命中：
     - `national`
   - 也就是“国家组优先布局”
   - 不再错误落到：
...
```text
...?target=ClashMeta&full&responseHeaders=true&regionGroups=apac,westasia,americas&groupOrderPreset=country&githubPreferCountries=apac-core&steamPreferCountries=westasia-core
```

## V9.5.0 这一轮新增了什么

这一轮继续沿着你现在这套玩法往前推，重点补的是 **更多区域/国家玩法** 和 **更实用的默认面板布局预设**：

1. **国家识别继续扩容**
   - 这次继续补了几类常见但之前还没收进去的节点国家：
     - `巴林`
     - `阿曼`
     - `约旦`
     - `白俄罗斯`
     ...
     - `巴拿马`
   - 目的很直接：
     - 让中东 / 海湾 / 东欧 / 地中海 / 东南亚补充分区一带的机场命名更容易落进国家组
     - 让后面的 `regionGroups` 和 `preferCountries` 也能吃到这些新增国家

2. **区域玩法继续细分**
   - `regionGroups` 这次继续补了几个更适合面板编排和优先链复用的子区域：
     - `apac`
     - `greaterchina`
     - `westasia`
     - `westeurope`
     ...
     - `levant`
   - 这样你现在既可以继续用大区：
     - `asia / europe / americas / middleeast`
   - 也可以往下细切成更贴近机场选线的子区域

3. **Prefer-Countries 预设继续增强**
   - 新增这些 preset：
     - `apac-core`
     - `westasia-core`
     - `workspace-core`
...
4. **默认面板布局预设继续增强**
   - `groupOrderPreset` 这次新增：
     - `workspace`
     - `compact`
     - `national`
   - 同时默认 `balanced` 也做了微调：
     - 会把 `regions / countries` 提前到主业务组后面
     - 区域组、国家组在默认面板里更容易直接看到
   - 含义分别是：
     - `workspace`：工作流优先，先把 AI / GitHub / 开发服务 / 微软 / OneDrive / Google 拉前
     - `compact`：紧凑面板，只保留最常切的业务组 + 国家/区域 + helpers
     - `national`：国家组优先，适合你更常直接按国家手选线路的场景

5. **这一轮可以直接这样用**

```text
...?target=ClashMeta&full&responseHeaders=true&regionGroups=greaterchina,apac,westeurope,dach,anglosphere,levant&groupOrderPreset=workspace&githubPreferCountries=workspace-core&steamPreferCountries=gulf-core&devPreferCountries=anglosphere-core
```

```text
...?target=ClashMeta&full&responseHeaders=true&regionGroups=greaterchina,apac,anglosphere,asia,europe&groupOrderPreset=national&githubPreferCountries=greaterchina-core&steamPreferCountries=asia-5
```
...
这一轮继续补“区域玩法”的完整性，把 **拉美 / 地中海** 两块也一起接上：

1. **新增两个子区域 token**
   - `latam`
     - 墨西哥 / 阿根廷 / 巴西 / 智利 / 哥伦比亚 / 秘鲁 / 乌拉圭
   - `mediterranean`
     - 西班牙 / 意大利 / 希腊 / 塞浦路斯 / 土耳其 / 以色列 / 埃及 / 摩洛哥 / 阿尔及利亚 / 突尼斯
...
```text
...?target=ClashMeta&full&responseHeaders=true&regionGroups=latam,mediterranean&githubPreferCountries=latam-core&steamPreferCountries=mediterranean-core&devPreferCountries=latam,mediterranean
```

## V9.0.0 这一轮新增了什么

这一轮继续把前面新增的区域真正收进 `PreferCountries` 预设体系，重点是让你不用每次手写一长串子区域 token：
...
2. **各 preset 的展开逻辑**
   - `middleeast-core`
     - `middleeast`
   - `africa-core`
     - `northafrica / 尼日利亚 / 肯尼亚 / 南非`
   - `mena-core`
     - `middleeast / northafrica`
   - `balkans-core`
     - `balkans`
   - `eurasia-core`
     - `eastasia / southasia / centralasia / caucasus`
...
```text
...?target=ClashMeta&full&responseHeaders=true&githubPreferCountries=balkans-core&steamPreferCountries=mena-core&devPreferCountries=eurasia-core&aiPreferCountries=middleeast-core
```

## V8.99.0 这一轮新增了什么

这一轮继续按你前面的方向，把 **北非 / 巴尔干** 这两个常见子区域补进来：
...
2. **新增两个子区域 token**
   - `balkans`
     - 希腊 / 保加利亚 / 罗马尼亚 / 塞尔维亚 / 阿尔巴尼亚 / 波黑 / 黑山 / 北马其顿 / 克罗地亚 / 斯洛文尼亚
   - `northafrica`
     - 埃及 / 摩洛哥 / 阿尔及利亚 / 利比亚 / 突尼斯
...
4. **继续保持缩写安全策略**
   - 这轮依然优先：
     ...
     - 三位缩写
     - 首都城市名
   - 尽量避免把高误判的两位缩写直接放进识别里

5. **直接可抄的示例**

```text
...?target=ClashMeta&full&regionGroups=balkans,northafrica&githubPreferCountries=balkans&steamPreferCountries=northafrica&devPreferCountries=阿尔巴尼亚,波黑,利比亚
```

## V8.98.0 这一轮新增了什么

这一轮继续按你前面要的方向，把 **国家库 + 区域子区域** 一起往下补：
...
```text
...?target=ClashMeta&full&regionGroups=centralasia,caucasus,africa&githubPreferCountries=centralasia&steamPreferCountries=caucasus&devPreferCountries=尼泊尔,斯里兰卡,乌拉圭
```

## V8.97.0 这一轮新增了什么

这一轮继续把 `PreferCountries` 从“能看到最终结果”往“能看到每个输入是怎么解析的”推进：
...
2. **新增 Explain 响应头**
   - 打开 `responseHeaders=true` 后，现在还会多出：
     - `AI-Prefer-Countries-Explain`
     - `Crypto-Prefer-Countries-Explain`
     - `GitHub-Prefer-Countries-Explain`
     - `Steam-Prefer-Countries-Explain`
     - `Dev-Prefer-Countries-Explain`
   - 用来看：
     - 每个 token 是按 `preset / region / country / fallback` 哪种方式命中的
     - 最终落到了哪些国家组

3. **新增 Unmatched 响应头**
   - 现在还会多出：
     - `AI-Prefer-Countries-Unmatched`
     - `Crypto-Prefer-Countries-Unmatched`
     - `GitHub-Prefer-Countries-Unmatched`
     - `Steam-Prefer-Countries-Unmatched`
     - `Dev-Prefer-Countries-Unmatched`
   - 用来看：
     - 哪些 token 当前没有匹配到已生成国家组

4. **`full` 日志新增两行**
   - `国家优先链解析`
   - `国家优先链未命中`
   - 这样不用只看 warning，也能直接看到每个 token 的解析结果和失败项

5. **直接可抄的示例**

```text
...?target=ClashMeta&full&responseHeaders=true&githubPreferCountries=classic-4,eastasia,foo&steamPreferCountries=gulf,美国,bar&devPreferCountries=日本,欧洲
```

## V8.96.0 这一轮新增了什么

这一轮继续把 `PreferCountries` 往“能看懂为什么这么排”推进：现在不只知道命中了哪些国家组，还能直接看到它们是从 **preset / region / country / fallback / default** 哪条来源展开出来的。
...
1. **新增国家优先链来源追踪**
   - 适用于：
     ...
     - `steamPreferCountries`
     - `devPreferCountries`
   - 现在除了最终命中摘要，还会额外保留来源类型：
     - `preset:classic-5`
     - `region:eastasia`
     - `country:日本`
     - `fallback:xxx`
     - `default:ai-default / crypto-default`
...
3. **响应头新增 trace 版本**
   - 打开 `responseHeaders=true` 后，除了原来的 `*-Resolved`，现在还会新增：
     - `AI-Prefer-Countries-Trace`
     - `Crypto-Prefer-Countries-Trace`
     - `GitHub-Prefer-Countries-Trace`
     - `Steam-Prefer-Countries-Trace`
     - `Dev-Prefer-Countries-Trace`

4. **preset 现在更方便排查**
   - 比如你写：
     - `githubPreferCountries=classic-4`
     - `steamPreferCountries=eastasia,gulf`
     - `devPreferCountries=日本,美国`
   - 现在能同时看到：
     - `Resolved`：最终命中了哪些国家组
     - `Trace`：这些组是从哪个 preset / region / country token 过来的

5. **直接可抄的示例**

```text
...?target=ClashMeta&full&responseHeaders=true&githubPreferCountries=classic-4&steamPreferCountries=eastasia,gulf&devPreferCountries=日本,美国
```

## V8.95.0 这一轮新增了什么

这一轮继续沿 GitHub 社区组合往下收，不只保留五地/六地，也把更轻量的四地组合补齐：
...
1. **新增四地 preset**
   - 新增：
     ...
     - `asia-4`
   - 对应展开：
     - `classic-4` → 香港 / 狮城 / 日本 / 美国
     - `asia-4` → 香港 / 台湾 / 狮城 / 日本

2. **兼容社区常见缩写**
   - 现在这些也能直接写进 `PreferCountries`：
     - `hksgjpus`
     - `hktwsgjp`
   - 以及带连字符版本：
     - `hk-sg-jp-us`
...
```text
...?target=ClashMeta&full&responseHeaders=true&githubPreferCountries=classic-4&steamPreferCountries=classic-5&devPreferCountries=asia-4
```

## V8.94.0 这一轮新增了什么

这一轮按你说的去 GitHub 检索了一圈社区项目，发现最稳定、最反复出现的还是这两组经典组合：

1. **新增 GitHub 社区经典五地 / 六地 preset**
   - 新增：
     - `classic-5`
     - `classic-6`
   - 对应展开：
     - `classic-5` → 香港 / 台湾 / 狮城 / 日本 / 美国
     - `classic-6` → 香港 / 台湾 / 狮城 / 日本 / 韩国 / 美国

2. **兼容社区常见连写缩写**
   - 现在这些也可以直接写进 `PreferCountries`：
     - `hktwsgjpus`
     - `hktwsgjpkrus`
   - 以及带连字符版本：
     - `hk-tw-sg-jp-us`
...
4. **继续继承现有体系**
   - 这些 preset 最终仍会继续复用：
     - `countryGroupSort`
     - `国家优先链命中`
     - `responseHeaders=true` 时的 resolved 摘要头

5. **直接可抄的示例**

```text
...?target=ClashMeta&full&responseHeaders=true&githubPreferCountries=classic-5&steamPreferCountries=classic-6&devPreferCountries=hktwsgjpkrus
```

## V8.93.0 这一轮新增了什么

这一轮继续往你前面想要的 **“少写一点，复用更多”** 推：`PreferCountries` 现在除了国家 token、区域 token，还支持一组可复用的 preset。

1. **新增 `PreferCountries` 预设包**
   - 现在可以直接写：
     - `ai-core`
     - `crypto-core`
     - `gaming-core`
...
2. **各 preset 的展开逻辑**
   ...
   - `crypto-core`
     - 日本 / 狮城 / 香港 / gulf
   - `gaming-core`
     - eastasia / southeastasia / northamerica
   - `dev-core`
     - eastasia / northeurope / centraleurope / northamerica
   - `asia-core`
     - eastasia / southeastasia / southasia
   - `europe-core`
     - northeurope / centraleurope
   - `americas-core`
     - northamerica / southamerica
   - `global-core`
     - eastasia / southeastasia / northeurope / centraleurope / northamerica / gulf

3. **preset 会继续递归展开**
   - 也就是说 preset 不是写死的一串最终国家名
   - 它会继续展开成：
     - 区域 / 子区域 token
     - 再落到当前已生成的国家组
   - 所以会自动继承：
...
```text
...?target=ClashMeta&full&responseHeaders=true&githubPreferCountries=gaming-core&steamPreferCountries=americas-core,gulf&devPreferCountries=dev-core&aiPreferCountries=ai-core
```
...
这一轮继续把你前面一直在推的 **“区域玩法和优先链别分家”** 收到一起，现在 `PreferCountries` 不只认国家，也能直接认区域 / 子区域 token：

1. **`PreferCountries` 现在支持区域 token**
   - 适用于：
     - `aiPreferCountries`
     - `cryptoPreferCountries`
     - `githubPreferCountries`
...
2. **区域 token 会自动展开成国家组**
   - 例如：
     - `githubPreferCountries=eastasia`
   - 会自动按当前已生成的国家组，展开成：
     - 香港 / 澳门 / 台湾 / 日本 / 韩国
   - 而且展开顺序会跟随当前国家组顺序，所以也会自动继承：
     - `countryGroupSort`

3. **不依赖 `regionGroups` 面板是否开启**
   - 就算你没有开：
     - `regionGroups`
   - 只要这些国家组本身已经生成出来，区域 token 也照样能用于 `PreferCountries`

4. **默认 AI / Crypto 优先链不变**
   - 这轮只增强你显式传入的 `PreferCountries`
   - 不会改掉脚本原本默认的：
     - AI 优先链
     - Crypto 优先链
...
```text
...?target=ClashMeta&full&countryGroupSort=count&githubPreferCountries=eastasia&steamPreferCountries=northamerica,gulf&devPreferCountries=southeastasia,centraleurope
```
...
3. **别名不只是识别国家，还会同步参与优先链匹配**
   ...
   - 同一轮里 `PreferCountries` 也已经支持直接写区域 token；两套能力可以叠加用
...
- `aiPreferCountries / cryptoPreferCountries / githubPreferCountries / steamPreferCountries / devPreferCountries` 现在也支持直接写 preset：
  ...
  - `baltics-core / nafta-core / southerncone-core`
...
- `PreferCountries` 的 preset 会先展开成区域 / 子区域 / 国家 token，再继续复用现有优先链解析逻辑
...
- 如果已经启用了 `regionGroups`，则 `githubPreferGroups / steamPreferGroups / devPreferGroups` 也可以直接写区域别名：
  - `asia / apac / greaterchina / eastasia / southeastasia / southasia / westasia`
  - `europe / westeurope / easteurope / northeurope / centraleurope / dach / iberia / benelux`
  - `americas / northamerica / southamerica / latam / anglosphere`
  - `middleeast / levant / gulf / oceania / africa / northafrica / mediterranean`
...
- `groupOrderPreset` 支持 `balanced / core / service / media / region / national / workspace / dashboard / compact / geo-compact`
...
- 工作流和紧凑布局支持：`workspace / work / office / workflow / workspace-first / dashboard / daily / panel / home / compact / compact-first / minimal / lite / geo-compact / subregion-first`
...
- 内置国家识别库已经继续补到 `巴林 / 阿曼 / 约旦 / 白俄罗斯 / 马耳他 / 老挝 / 巴拿马 / 黎巴嫩 / 哥斯达黎加 / 厄瓜多尔 / 巴拉圭 / 玻利维亚 / 巴基斯坦 / 孟加拉 / 哈萨克 / 柬埔寨 / 文莱 / 塞尔维亚 / 摩尔多瓦 / 塞浦路斯 / 尼日利亚 / 摩洛哥 / 肯尼亚` 等常见节点命名
...
- `regionGroups` 支持：
  ...
  - `europe / westeurope / easteurope / northeurope / centraleurope / dach / balkans / iberia / benelux / baltics`
  - `americas / northamerica / nafta / southamerica / southerncone / latam / anglosphere`
...
- 如果已经启用了 `regionGroups`，`groupOrder` 还支持直接写区域别名：
  - `asia / apac / greaterchina / eastasia / southeastasia / southasia / centralasia / caucasus / westasia`
  - `europe / westeurope / easteurope / northeurope / centraleurope / dach / iberia / benelux / baltics / balkans`
  - `americas / northamerica / nafta / southamerica / southerncone / latam / anglosphere`
  - `middleeast / levant / gulf / oceania / africa / northafrica / mediterranean`
...
- 从 `preset` 展开的
- 还是从 `region` / `subregion` 展开的
- 还是从 `country` token 直接命中的
- 或者最后落到了 `fallback` / `default`
- 以及每个 token 是否未命中
...
githubPreferCountries=eastasia
steamPreferCountries=northamerica,gulf
devPreferCountries=southeastasia,centraleurope
```

### 2C. 用 preset 写优先链

```ini
aiPreferCountries=ai-core
githubPreferCountries=gaming-core
steamPreferCountries=americas-core,gulf
devPreferCountries=dev-core
```

### 2C+. 用区域核心 preset 写优先链

```ini
aiPreferCountries=middleeast-core
githubPreferCountries=balkans-core
steamPreferCountries=mena-core
devPreferCountries=eurasia-core,baltics-core
```

### 2C++. 用拉美 / 地中海 preset 写优先链

```ini
githubPreferCountries=latam-core
steamPreferCountries=mediterranean-core
devPreferCountries=latam,mediterranean,southerncone
```

### 2D. 用 GitHub 社区经典五地 / 六地写优先链

```ini
githubPreferCountries=classic-5
steamPreferCountries=classic-6
devPreferCountries=hktwsgjpkrus
```

### 2E. 用四地轻量 preset 写优先链

```ini
githubPreferCountries=classic-4
steamPreferCountries=classic-5
devPreferCountries=asia-4
```

### 2F. 混合查看 Resolved + Trace

```ini
full&responseHeaders=true&githubPreferCountries=classic-4&steamPreferCountries=eastasia,gulf&devPreferCountries=日本,美国
```

### 2G. 混合查看 Explain + Unmatched

```ini
full&responseHeaders=true&githubPreferCountries=classic-4,eastasia,foo&steamPreferCountries=gulf,美国,bar&devPreferCountries=日本,欧洲
```
...
```ini
regionGroups=centralasia,caucasus,africa&githubPreferCountries=centralasia&steamPreferCountries=caucasus
```

### 9E. 巴尔干 / 北非面板

```ini
regionGroups=balkans,northafrica&githubPreferCountries=balkans&steamPreferCountries=northafrica
```

### 9F. 拉美 / 地中海面板

```ini
regionGroups=latam,mediterranean&githubPreferCountries=latam-core&steamPreferCountries=mediterranean-core
```
