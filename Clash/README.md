...
- 自动规范节点名称、处理重复名
- 自动过滤“TG频道 / 剩余流量 / 套餐到期 / 官网 / 更新地址 / 回家地址 / 备用网址 / 工单群 / 客服群 / 防失联 / 地址发布页 / 最新域名”等信息节点，不再保留到最终节点列表
- 国家识别与国家分组
- 区域分组与区域别名
...
这一版把垃圾信息过滤词表做成了更好维护的结构：

- 主脚本里不再直接手写一整段超长正则，改成“关键词数组 + 自动拼装正则”
- 过滤行为保持不变，但后续要增删词时只需要维护词表本身
- 固定模板 `Nikki_fallback_Rule-Set.yaml` 继续保留同步后的扩展过滤范围

### V9.14.66

这一版不再零散补词，直接把机场说明节点词表整编了一轮：

- 一次性补齐了地址发布、社群通知、客服引导、订阅说明、流量到期、失联补救这几类高频垃圾文案
- 现在像 `TG频道 / 官网 / 更新地址 / 回家地址 / 永久地址 / 地址发布页 / 最新域名 / 客服群 / 官方群 / 防失联 / 套餐到期 / 流量重置` 这类名字都会在节点规范化阶段直接丢弃
- 固定模板 `Nikki_fallback_Rule-Set.yaml` 的订阅过滤正则也同步扩成同一风格，避免脚本版和固定模板版长期漂移

### V9.14.65

这一版继续扩了机场垃圾信息关键词：

- 新增 `客服群 / 防失联 / 最新域名 / 地址发布页` 等常见机场通知文案
- `TG频道 / 官网 / 更新地址 / 备用网址 / 工单群 / 客服群 / 防失联 / 最新域名 / 地址发布页` 这类名字会在节点规范化阶段直接丢弃
- 固定模板 `Nikki_fallback_Rule-Set.yaml` 的订阅过滤正则也同步纳入这批关键词

### V9.14.64

这一版继续补强了垃圾信息节点的识别范围：

- 新增 `备用网址 / 更新地址 / 工单群` 等常见机场说明关键词
- `TG频道 / 套餐到期 / 官网 / 更新地址 / 备用网址 / 工单群` 这类名字会在节点规范化阶段直接丢弃
- 固定模板 `Nikki_fallback_Rule-Set.yaml` 的订阅过滤正则也同步补强，避免两条配置线行为不一致

### V9.14.63

这一版把机场说明类节点从“单独收口”改成了“直接过滤”：

- `TG频道 / 剩余流量 / 套餐到期 / 官网` 这类名字会在节点规范化阶段直接丢弃
- 不再生成 `ℹ️ 订阅信息` 组
- `full` 摘要里仍会保留“已过滤信息节点”数量，方便确认过滤是否命中

### V9.14.62

这一版补强了机场说明类节点的识别范围：

- 新增 `频道 / 套餐 / 余量 / 公告 / 通知 / 重置 / t.me` 等信息关键词
- `TG频道 / 剩余流量 / 套餐到期 / 官网` 这类名字会命中信息节点识别规则
- 这类节点不会参与国家分组、自动测速组和其他自动收集组
