 ...
 * 336. 港区流媒体规则继续审计：参考 blackmatrix7 当前目录，把 NowE 并入流媒体组，补上 nowe.com / nowestatic.com；TVer / HamiVideo 仍暂不纳入，避免把更宽的地区媒体域名一起卷入。
 * 337. 台区流媒体规则继续审计：参考 blackmatrix7 当前目录，把 KKTV 并入流媒体组，补上 kktv.com.tw / kktv.me；LineTV / HamiVideo 仍暂不纳入，避免把更宽的 LINE/Hinet 生态域名一起卷入。
...
function resolveArgs(rawArgs) {
  ...
  return {
    ...
    extraDirectDomains,
    hasExtraDirectDomains: !!extraDirectDomains.length,
...
const ruleProviders = finalizeRuleProviders({
  ...
  // HashiCorp / Terraform / Consul / Vault 基础设施即代码规则。
  HashiCorp: createDeveloperRuleProvider("HashiCorp"),
  // Unity / UnityHub / Unity China 游戏引擎与编辑器分发规则。
  Unity: createDeveloperRuleProvider("Unity"),
  ...
  // UCloud / UFile / UCDN 云平台、对象存储与 CDN 管理规则。
  UCloud: createDeveloperRuleProvider("UCloud"),
...
const RULE_SET_DEFINITIONS = (() => {
  ...
  // HashiCorp / Terraform / Consul / Vault 基础设施即代码流量交给开发服务组。
  { provider: "HashiCorp", target: GROUPS.DEV, overrideKey: "devRuleTarget", overrideFlagKey: "hasDevRuleTarget", overrideLabel: "Dev" },
  // Unity / UnityHub / Unity China 游戏引擎与编辑器流量交给开发服务组。
  { provider: "Unity", target: GROUPS.DEV, overrideKey: "devRuleTarget", overrideFlagKey: "hasDevRuleTarget", overrideLabel: "Dev" },
  ...
  // UCloud / UFile / UCDN 云平台、对象存储与 CDN 管理流量交给开发服务组。
  { provider: "UCloud", target: GROUPS.DEV, overrideKey: "devRuleTarget", overrideFlagKey: "hasDevRuleTarget", overrideLabel: "Dev" },
...
function buildRules(quicEnabled, ruleDefinitions) {
  ...
  const extraDirectDomainRules = buildExtraDirectDomainRules(ARGS.extraDirectDomains);
  let insertedExtraDirectDomainRules = false;
  ...
  for (const definition of definitions) {
    ...
    // 手工追加的直连域名放在 Private / Private_IP 之后、业务规则之前，保持“广告 -> 私有/本地 -> 手工直连 -> 业务分流”的顺序。
    if (!insertedExtraDirectDomainRules && definition && definition.provider === "Private_IP" && extraDirectDomainRules.length) {
      rules.push.apply(rules, extraDirectDomainRules);
      insertedExtraDirectDomainRules = true;
    }
  }

  if (!insertedExtraDirectDomainRules && extraDirectDomainRules.length) {
    rules.unshift.apply(rules, extraDirectDomainRules);
  }
...
function buildProxyGroupBaseContext(payload) {
  ...
  return {
    ...
    infoProxyNames: collectInfoProxyNames(context.proxies),
...
const RULE_PROVIDER_ALIAS_MAP = Object.freeze({
  ...
  consul: "HashiCorp",
  packer: "HashiCorp",
  nomadproject: "HashiCorp",
  vagrant: "HashiCorp",
  vaultproject: "HashiCorp",
  unity: "Unity",
  unity3d: "Unity",
  unityhub: "Unity",
  unitychina: "Unity",
...
// 订阅公告/信息类节点只适合单独查看，不适合混进测速与国家分组。
...
// 归一化用户传入的“额外直连域名”条目，兼容裸域名、带通配前缀与完整 URL。
function normalizeDirectDomainEntry(entry) {
  let value = normalizeStringArg(entry);
  if (!value) {
    return "";
  }

  if (typeof URL === "function" && /^[a-z]+:\/\//i.test(value)) {
    try {
      value = normalizeStringArg(new URL(value).hostname);
    } catch (error) {
      // URL 解析失败时继续按原始字符串兜底处理。
    }
  }
...
// 额外直连域名参数统一清洗成字符串数组，后续规则与 fake-ip-filter 直接复用。
function normalizeDirectDomainEntries(value) {
  return uniqueStrings(toStringList(value).map((entry) => normalizeDirectDomainEntry(entry)).filter(Boolean));
}

// 额外直连域名默认按 DOMAIN-SUFFIX 处理；显式传入 . / +. / *. 时同样折叠成后缀匹配。
function buildExtraDirectDomainRule(entry) {
  const value = normalizeDirectDomainEntry(entry);
  if (!value) {
    return "";
  }

  return `DOMAIN-SUFFIX,${value.replace(/^(\+\.|\*\.|\.)/, "")},${GROUPS.DIRECT}`;
}

// 把参数里的额外直连域名转换成最终规则，统一插进脚本生成规则链。
function buildExtraDirectDomainRules(entries) {
  return uniqueStrings((Array.isArray(entries) ? entries : []).map((entry) => buildExtraDirectDomainRule(entry)).filter(Boolean));
}
...
function analyzeProxies(proxies) {
  ...
  for (const proxy of Array.isArray(proxies) ? proxies : []) {
    ...
    // 说明/公告类节点单独计数，便于 full 日志快速确认是否被成功隔离。
    if (isInfoProxyName(name)) {
      stats.info += 1;
    }
  }
...
function createContextLatencyGroupBuildDefinition(groupName, contextKey, options) {
  const currentOptions = isObject(options) ? options : {};
  return {
    build: (context) => createProxyListLatencyGroup(
      groupName,
      context[contextKey],
      currentOptions.type,
...
const PROXY_GROUP_EXTRA_GROUP_DEFINITION_SECTIONS = Object.freeze([
  Object.freeze([
    ...
    createConditionalProxyGroupBuildDefinition(
      ...
      (context) => createIncludeAllLatencyGroup(
        GROUPS.LOW_COST,
        composeCaseInsensitivePattern([REGEX_LOW_COST.source]),
        context.infoExcludeFilter,
        "url-test"
      )
    ),
...
const PROXY_GROUP_RUNTIME_CONTEXT_DEFINITIONS = Object.freeze([
  {
    key: "infoExcludeFilter",
    value: (context) => Array.isArray(context.infoProxyNames) && context.infoProxyNames.length
      ? composeCaseInsensitivePattern([REGEX_INFO_NODE.source])
      : ""
  },
  {
  ...
  },
  {
    ...
    value: (context) => buildProxyGroupGeneratedGroups({
      ...
      infoProxyNames: context.infoProxyNames,
      infoExcludeFilter: context.infoExcludeFilter,
...
function buildDnsConfig(existingDns) {
  ...
  dns["fake-ip-filter"] = buildFakeIpFilter(
    fakeIpFilter.concat(ARGS.extraDirectDomains),
    currentDns["fake-ip-filter"],
    scalarOptions.fakeIpFilterMode
  );
